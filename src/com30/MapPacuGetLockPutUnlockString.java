...
public class MapPacuGetLockPutUnlockString {
	private static List<Thread> getSetCollectionThreads;
	private static AtomicInteger a = new AtomicInteger(0);

	public static void main(String[] args) {
		JCLHashMapPacu<Object, Integer> mapPacu = new JCLHashMapPacu<Object, Integer>("stringmap");
					...
					try {
						...
						while((resource.isFinished()==false) && (resource.getNumOfRegisters()!=0)) {
							if((varName = resource.getRegister()) != null) {
								a.set(a.get() + 1);
								System.out.println(Thread.currentThread().getId() + " " + a.get() + ": " + mapPacu.getLock("string" + varName));
								mapPacu.putUnlock("string" + varName, Integer.parseInt(varName));
//								jcl.getValueLocking("string" + varName).getCorrectResult();
//								jcl.setValueUnlocking("string" + varName, Integer.parseInt(varName));
							}
						}
