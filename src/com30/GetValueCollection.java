...
public class GetValueCollection {
					...
					try {
						...
						while((resource.isFinished()==false) && (resource.getNumOfRegisters()!=0)) {
							if((varName = resource.getRegister()) != null) {
								a.set(a.get() + 1);
								System.out.println(Thread.currentThread().getId() + " " + a.get() + ": " + jcl.getValue("collection" + varName).getCorrectResult());
//								jcl.getValue("collection" + varName).getCorrectResult();
							}
						}
